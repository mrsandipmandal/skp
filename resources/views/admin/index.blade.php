        ...
        <div class="row gy-6">
            ...
            <div class="col-md-12 col-lg-4">
                <div class="card">
                    ...
                    <img src="{{ url('/') }}/assets/img/illustrations/trophy.png" class="position-absolute bottom-0 end-0 me-5 mb-5"
                        ...
                        <div class="card-body card-separator">
                            ...
                            <div class="deposit-content pt-2">
                                <ul class="p-0 m-0">
                                    ...
                                    <li class="d-flex mb-4 align-items-center pb-2">
                                        <div class="flex-shrink-0 me-4">
                                            <img src="{{ url('/') }}/assets/img/icons/payments/mastercard-2.png" class="img-fluid"
                                    ...
                                    <li class="d-flex mb-4 align-items-center pb-2">
                                        <div class="flex-shrink-0 me-4">
                                            <img src="{{ url('/') }}/assets/img/icons/payments/american-bank.png" class="img-fluid"
                        ...
                        <div class="card-body">
                            ...
                            <div class="withdraw-content pt-2">
                                <ul class="p-0 m-0">
                                    ...
                                    <li class="d-flex mb-4 align-items-center pb-2">
                                        <div class="flex-shrink-0 me-4">
                                            <img src="{{ url('/') }}/assets/img/icons/payments/digital-ocean.png" class="img-fluid"
                                    ...
                                    <li class="d-flex align-items-center">
                                        <div class="flex-shrink-0 me-4">
                                            <img src="{{ url('/') }}/assets/img/icons/brands/aws.png" class="img-fluid"
...
@section('script_div')
    <script>
        $(document).ready(function() {
            ...
            async function filter() {
                ...
                progress.style.width = '1%'; // Start animation
                setTimeout(() => {
                    progress.style.width = '100%'; // Complete animation
                ...
                if (!formData.proj && !formData.block && !formData.mouza) {
                    alert("Please Select Project, Block or Mouza");
                    return;
                }

                // Prepare query parameters for the GET request
                const queryParams = new URLSearchParams(formData).toString();
                const apiUrl = `{{ url('/api/dash-filter') }}?${queryParams}`;
                ...
                try {
                    ...
                    if (result.error) {
                        ...
                        // required_land
                        $('#required_land').text(result.required_land);
                        $('#required_done').text(result.required_done);
                        $('#required_pending').text(result.required_pending);
                        // required_land
                        $('#total_beneficiary').text(result.total_beni);
                        $('#pending_beneficiary').text(result.assign_beni);
                        $('#assign_land').text(result.beni_pending);
                        // required_land
                        $('#total_notice').text(result.total_notice);
                        $('#notice_pending').text(result.notice_pending);
                        $('#done_notice_beni').text(result.done_notice_beni);
                        // required_land
                        $('#registration_ttl').text(result.registration_ttl);
                        $('#registration_pending').text(result.registration_pending);
                        $('#registration_done').text(result.registration_done);
                        // required_land
                        $('#total_fund_received').text(result.total_fund_received);
                        $('#total_payment_released').text(result.total_payment_released);
                        $('#balance_fund').text(result.balance_fund);

                        setTimeout(() => {
                            progressBar.style.display = 'none';
                            progress.style.width = '0%'; // Reset progress bar
                        }, 1000); // Adjust timing based on your redirection behavior
            ...
            // Handle change to trigger AJAX load for the first page
    ...
    @if (session()->has('beneficiary_typ') && session()->has('beneficiary_message'))
        <script>
            Swal.fire({
                position: 'center',
                icon: '{{ session('beneficiary_typ') }}',
                title: '{{ session('beneficiary_message') }}',
    ...
    <script>
        async function getBlockData() {
            try {
                ...
                const selectedBlockId = {{ $selectedBlockId ?? 'null' }}; // Use the value passed from the backend

                // Construct the API URL with query parameters
                const apiUrl = `{{ url('/api/getProjectBlock?proj=${proj}') }}`;
                const response = await fetch(apiUrl, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                ...
                if (result.error || result.data === "Data Not Found.") {
                    alert('No data found.');
                    return;
                }

                const dropdown = document.getElementById('block');
                dropdown.innerHTML = '<option value="">Select a block</option>'; // Clear existing options

                // Populate the dropdown with the data
                result.data.forEach(block => {
                    const option = document.createElement('option');
                    option.value = block.sl; // Assuming 'sl' is the block ID
                    option.textContent = block.bnm; // Assuming 'bnm' is the block's display name

                    // Pre-select the option if it matches the selectedBlockId
                    if (block.sl == selectedBlockId) {
                ...
                // After populating blocks, call getGpData to populate GP dropdown if block is preselected
                if (selectedBlockId) {
                    getGpData(); // Load the GP data for the selected block
                }
            } catch (error) {
        ...
        async function getGpData() {
            try {
                ...
                const selectedGpId = {{ $selectedGpId ?? 'null' }}; // Use the value passed from the backend

                const apiUrl = `{{ url('/api/getMouza1?block=${block}') }}`;
                const response = await fetch(apiUrl, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                ...
                const dropdown = document.getElementById('mouza');
                dropdown.innerHTML = '<option value="">Select a GP/Ward</option>'; // Clear existing options
                // Populate the GP dropdown with the data
                result.data.forEach(gp => {
                    gp.get_m_o_u_z_a.forEach(xx => {
                        const option = document.createElement('option');
                        option.value = xx.sl; // Assuming 'sl' is the GP ID
                        option.textContent = xx.mouza; // Assuming 'pnm' is the GP's display name

                        // Pre-select the option if it matches the selectedGpId
                        if (gp.sl == selectedGpId) {
                            option.selected = true;
                        }
        ...
        async function getMouzaData() {
            try {
                var proj = document.getElementById('proj').value;
                var mouza = document.getElementById('mouza').value;
                var block = document.getElementById('block').value;

                const selectedGpId = {{ $selectedGpId ?? 'null' }}; // Use the value passed from the backend

                const apiUrl = `{{ url('/api/getvalbeni') }}?proj=${proj}&mouza=${mouza}&block=${block}`;
                const response = await fetch(apiUrl, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                ...
                // add remaning land
                $('#remd').html("<font size='1px' color='red'>(Remanding Land in Satak : " + result.val + ")</font>");
                // $('#rtlis').val(result.val);
                //
